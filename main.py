...
from config import Token, smtp_password

SMTP_PASSWORD = smtp_password
SMTP_SERVER = 'smtp.gmail.com'
SMTP_PORT = 587
SMTP_USERNAME = 'bagget7777@gmail.com'
DB_FILE = 'bot.db'
...
async def email_handler(message: types.Message):
    ...
    await message.reply("Отлично! Теперь укажи формат, в котором хочешь получить видео.", reply_markup=get_format_keyboard())


@dp.message_handler(commands=['verify_buttons'])
async def verify_buttons_handler(message: types.Message):
    await message.reply("Выберите формат видео:", reply_markup=get_format_keyboard())
...
async def errors_handler(update, exception):
    print(f'Ошибка при обработке сообщения: {update}\n{exception}')


@dp.message_handler()
async def unknown_message_handler(message: types.Message):
    await message.reply("Неизвестная команда или сообщение. Пожалуйста, воспользуйтесь доступными командами.")
